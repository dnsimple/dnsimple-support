...
excerpt: What name servers are, why they matter, and how they work in the DNS system.
meta: Name servers translate domain names into IP addresses, forming the foundation of DNS resolution and enabling internet connectivity.
categories:
- Name Servers
---
...
Name servers are specialized servers that form the backbone of the Domain Name System (DNS). They translate human-readable domain names (like `example.com`) into machine-readable IP addresses (like `192.0.2.1`), enabling computers to locate and connect to websites, email servers, and other internet services.
...
## The role of name servers in DNS {#role}

When you type a domain name into your browser, your device queries name servers to find the corresponding IP address. This process, known as DNS resolution, involves multiple steps:
...
## Authoritative vs. recursive name servers {#types}
...
## How name servers relate to domain delegation {#delegation}
...
For example, if you delegate your domain to DNSimple's name servers (`ns1.dnsimple.com`, `ns2.dnsimple-edge.net`, etc.), all DNS queries for your domain will be answered by DNSimple's authoritative name servers, using the DNS records you have configured in your DNSimple account.

## Managing your domain's name servers {#managing}
...
To change your domain's name servers to point to a different DNS provider, see [Setting the Name Servers for a Domain](/articles/setting-name-servers/).

> [!NOTE]
> Verify that your domain is properly delegated to the correct name servers. If your domain is not delegated to DNSimple, [changes you make to DNS records](/articles/record-editor/) will not resolve. You can use tools like [zone.vision](https://zone.vision/#/) to check which name servers your domain is using.

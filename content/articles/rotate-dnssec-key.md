...
The [key signing keys and zone signing key](/articles/types-of-dnssec-keys/) rotation period starts 83 days after they are created and can last between 2 and 14 days. DNSSEC key set rotation is automatic and mandatory. It cannot be disabled.

Here is what you need to do, depending on how your domain is set up.

## If your domain is registered with us {#if-your-domain-is-registered-with-us}
...
## If your domain is not registered with us {#if-your-domain-is-not-registered-with-us}

You will need to manually manage your DS records at your domain registrar.

**How it works:**

1. When key rotation starts, we will send you an email notification with the new DS record details. You will also see the DS record details in the DNSSEC tab while the key rotation is ongoing. At this point, your zone will be signed by two sets of DNSSEC keys.
1. You have 7 days to add the DS records at your registrar.
1. Once our system verifies the new DS record is fully present in the parent zone, we will send you another email notification with the old DS record details.

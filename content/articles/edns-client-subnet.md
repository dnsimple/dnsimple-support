...
By default, a DNS query from a public resolver (like Google Public DNS, 8.8.8.8) only shows the resolver's IP address. This can be problematic if the resolver is geographically far from the user, causing the DNS response to route the user to a server that is not close to them. ECS solves this by allowing the resolver to pass along a partial, masked version of the end user's IP address in the DNS query.

## How it benefits your services {#how-it-benefits-your-services}
...
## How DNSimple supports ECS {#how-dnsimple-supports-ecs}

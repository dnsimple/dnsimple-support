...
## CNAME record format {#cname-record-format}
...
## CNAME record restrictions {#cname-record-restrictions}
Understanding the following restrictions is critical to properly using CNAME records:
- **No coexistence with other records:** A CNAME record cannot coexist with any other record for the same name. It is not possible to have both a CNAME and an [MX record](/articles/mx-record/) for `www.example.com` simultaneously.

...
Domain delegation is the process of assigning authority for a domain's DNS to specific name servers. When you delegate a domain, you are telling the internet where to find the DNS records for your domain.
...
## How does delegation work? {#how-does-delegation-work}
...
## Name servers and delegation {#name-servers-and-delegation}

Name servers are the DNS servers that are authoritative for your domain. When you delegate a domain, you are specifying which name servers should be queried for your domain's DNS records. Many registrars provide their own name servers that you can use for [DNS hosting](/articles/dns-hosting/). You can delegate to name servers provided by specialized DNS hosting services. Some organizations set up their own name servers (called [vanity name servers](/articles/what-are-vanity-name-servers/)) for branding or control.

## Delegation vs. registration {#delegation-vs-registration}

It is important to understand that delegation is separate from domain registration. Registration means owning the domain name and having the right to use it. Delegation means specifying which name servers handle the DNS for your domain. You can register a domain with one registrar and delegate it to name servers from a different DNS provider. This separation allows you to choose the best registrar for registration and the best DNS provider for hosting.
...
## Delegation and domain transfers {#delegation-and-domain-transfers}
...
- [How Name Server Delegation Works](/articles/how-name-server-delegation-works/): A deeper look at how DNS resolvers follow the delegation chain through the DNS hierarchy.
- [Pointing a Domain to DNSimple](/articles/pointing-domain-to-dnsimple/): How to delegate your domain to DNSimple's name servers.
- [Setting the Name Servers for a Domain](/articles/setting-name-servers/): How to change your domain's name servers in DNSimple.

...
1.  If the domain is already in your account, on the top-nav menu, click the <label>Certificates</label> tab, then the <label>Add new</label> button on the upper right.
1.  From the drop-down under Sectigo, locate the domain you are purchasing the certificate for. Click <label>Continue</label>.
...
1.  Configure the certificate details:
    ...
    1.  Select the signature algorithm. ECDSA is selected by default, but you can select RSA if your scenario requires it.
    1.  Leave the CSR option unchecked, unless a [custom CSR](/articles/what-is-csr/) is needed. The easiest option is to have DNSimple automatically generate the CSR and a new private key. Make sure to read our [private key policy](https://dnsimple.com/private-key-policy).

1.  Click <label>Purchase certificate</label> if you have a subscription.

    ![Purchase a Certificate](/files/dnsimple-certificate-purchase.png)

    If you do not have a subscription, click <label>Continue payment</label>.

    ![Purchase a Certificate no subscription](/files/purchase-ssl-certificate-continue-payment.png)

1.  Once the certificate is paid for, select the validation email from the list and click <label>Send validation email</label>.

    ![Select validation email](/files/select-validation-email.png)

1.  Follow the steps in the validation email to complete domain validation.

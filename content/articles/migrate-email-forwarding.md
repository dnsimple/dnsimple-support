...
## Step 1: Document your current configuration {#document}
...
> [!TIP]
> Create a spreadsheet or document listing each email address (From), its destination (To), and any special requirements.

## Step 2: Set up email forwarding in DNSimple {#setup}
...
1. Click the <label>Email Forwarding</label> tab.
1. For each email forward from your documentation:
   - Click <label>Add email forward</label>
   - Enter the local part in the <label>From</label> field (e.g., `hello` for `hello@yourdomain.com`)
   - Enter the full destination email address in the <label>To</label> field
   - Click <label>Add email forward</label>
1. If you need a catch-all, select <label>Catch-all</label> or enter `(.*)` in the <label>From</label> field, then enter the destination email address.
</div>
...
## Step 3: Verify DNS configuration {#verify-dns}
...
## Step 4: Test email forwarding {#test}
...
## Step 5: Update dependent services {#update-services}
...
## Step 6: Cancel old service {#cancel}
...
4. **Verify old MX records removed:** Ensure the old provider's MX records are no longer present
...
5. **Contact support:** If issues persist, [contact DNSimple support](https://dnsimple.com/feedback)

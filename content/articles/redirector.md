...
---
> [!NOTE]
> **HTTP** redirects are available on all plans. **HTTPS** redirects require a [Teams plan or higher](/articles/dnsimple-plans/#feature-comparison).
...
This feature cannot be used to redirect from one protocol to another protocol on the same host name. For example, you cannot use it to redirect from `http://example.com` to `https://example.com`. To perform a redirect from HTTP to HTTPS on the same host, you will need to implement the redirection inside your application. If you host on Heroku, [this guide](/articles/redirect-heroku/#redirect-http2https) explains how to perform the redirection.


## Configuring a redirect {#configuring-a-redirect}
...
## Redirecting status code {#redirecting-status-code}

The redirector sets a 301 status code. The code is not configurable, and it is not possible to return a 302 temporary redirect using the `URL` record.
...
> [!WARNING]
> Do not delete the private key of SSL certificates required for HTTPS redirects. If you delete the private key, the redirector will not be able to support HTTPS redirects for the associated domain names.
...
## Path-scoped redirects {#path-scoped-redirects}

Only bare domains or subdomains can be redirected, and they can be directed to a URL that either does or does not specify a path. It is not possible to specify a path in the redirect source.
...
Any path or query information passed by the user is passed to the resulting URL. If you set up a redirect from `blog.example.com` to `example.com`, when the user goes to `blog.example.com/awesome_article` they will be redirected to `example.com/awesome_article`.


## Wildcard URL redirects {#wildcard-url-redirects}

You can configure a wildcard redirect using the same conventions of a DNS wildcard record. However, the target cannot contain any wildcard references.
...
## Match-based URL redirects {#match-based-url-redirects}

The redirector does not support redirect targets that include a back-reference (match) to the redirect source. For example, the following redirects will not work:

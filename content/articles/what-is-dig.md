...
**`dig`** is a command-line tool used for querying the [Domain Name System (DNS)](/articles/what-is-dns/). It stands for **Domain Information Groper**. It is a powerful utility for diagnosing DNS issues and gathering information about DNS records.

`dig` gives you a direct window into the DNS. It lets you ask a DNS server specific questions about a domain and returns a detailed response, which can be invaluable for troubleshooting problems like incorrect IP addresses, email delivery issues, or domain delegation errors.

## How `dig` works {#how-dig-works}

When you run a `dig` command, you instruct your computer to perform a direct query against a DNS server. The command allows you to specify exactly what information you are looking for, such as the record type ([`A`](/articles/a-record/), [`CNAME`](/articles/cname-record/), [`MX`](/articles/mx-record/), etc.) and the specific DNS server you want to query.
...
## `dig`'s role in troubleshooting {#digs-role-in-troubleshooting}

The detailed output of `dig` provides the following key information:
...
- **Status codes**: Provides a status (e.g., `NOERROR`, `NXDOMAIN`) that tells you whether the query was successful, if the domain does not exist, or if there was a server error.

Analyzing this information lets you pinpoint exactly where a DNS problem lies—whether it is an incorrect record, an unresponsive name server, or a caching issue.

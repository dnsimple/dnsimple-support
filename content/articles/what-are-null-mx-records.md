...
excerpt: What null MX records are, why they matter for security, and when to use them.
meta: Understand null MX records and how they prevent backscatter and improve email security for domains that do not accept email.
categories:
- Emails
- DNS
---
...
A **null MX record** is a special type of MX record that explicitly indicates a domain does not accept email. This prevents mail servers from attempting to deliver email to the domain and helps protect against backscatter and other email-related issues.

## Understanding null MX records {#understanding}
...
## Why use null MX records? {#why-use}

### Prevent backscatter {#backscatter}

**Backscatter** occurs when a mail server receives an email that it cannot deliver and sends a bounce message (non-delivery report) back to the sender. If the original email was spoofed (had a fake sender address), the bounce message goes to an innocent third party.

Null MX records prevent this by telling mail servers upfront that the domain does not accept email, so they will not attempt delivery and will not generate bounce messages.

### Security benefits {#security}

Null MX records provide several security benefits:

- **Prevent email spoofing:** By explicitly stating a domain does not accept email, you reduce the risk of your domain being used in email spoofing attacks.
...
### Compliance and best practices {#compliance}

Using null MX records is considered a best practice for domains that do not need email functionality. It is a clear, standards-based way to communicate that a domain does not accept email.

## When to use null MX records {#when-to-use}

Use null MX records when:

- **Your domain does not need email:** You are using the domain only for websites, APIs, or other non-email purposes.
- **You want to prevent email delivery attempts:** You want to explicitly tell mail servers not to attempt delivery.
- **You are concerned about security:** You want to reduce the risk of email-related security issues.
- **You are following best practices:** You want to follow RFC 7505 recommendations for domains that do not accept email.

> [!WARNING]
> Only use null MX records if you are certain your domain will never need to receive email. If you might need email in the future, it is better to simply not have any MX records rather than using a null MX record.

## When not to use null MX records {#when-not-to-use}
...
- **You might need email in the future:** If there is any possibility you will need email functionality, do not use null MX records.

## How null MX records work {#how-it-works}
...
## Null MX records vs. no MX records {#vs-no-mx}

There is an important difference between having a null MX record and having no MX records at all:

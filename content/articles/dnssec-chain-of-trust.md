...
The chain of trust is the fundamental mechanism that lets DNSSEC-aware resolvers verify the authenticity and integrity of DNS data cryptographically. It ensures the information they receive has not been tampered with and comes from a trusted, authoritative source.

## What is the DNSSEC chain of trust? {#what-is-the-dnssec-chain-of-trust}
The DNSSEC chain of trust is a cryptographic pathway that follows the hierarchical structure of the Internet's DNS. It is a series of digital signatures that link each level of the DNS system, from the root zone down to Top-Level Domains (TLDs) and individual domains, through all the steps in between.
...
## Why is a chain of trust needed? {#why-is-a-chain-of-trust-needed}
The chain of trust is essential to your domain name security with DNSSEC. Without a chain of trust, a DNSSEC-validating resolver would not know where to begin trusting DNS data.
...
## How the chain of trust works {#how-the-chain-of-trust-works}
...
1. **Setting up trust (the role of DS records in secure delegation)**
    ...
    A parent zone does not directly sign the child zone's data. Instead, to establish a secure delegation, the parent zone signs a cryptographic digest (a hash) of the child zone's KSK. This summary is called a DS record.

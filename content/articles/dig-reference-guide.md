...
This document serves as a quick reference for the `dig` command-line tool, its syntax, and its output. To see how to use `dig`, check out [How to Use dig](https://support.dnsimple.com/articles/how-dig/).
...
- **HEADER**: Provides information about the query and response, such as the status code. A status of `NOERROR` indicates a successful query.
...
- **ADDITIONAL SECTION**: Provides other records that may be relevant to the query, such as the IP addresses of the authoritative name servers listed in the AUTHORITY section.
